...
function setRegions() 
{
  console.log('set regions passed!!');
  let countryOptions ='<option value="" selected="selected">SELECT REGION</option>'+
  '<option value=""></option>';
  for (region in countries){
    countryOptions +='<option value="' + region + '">' + region + '</option>';
  }
  return countryOptions;
}


function set_country(targetRegion, oCountrySel, oCity_StateSel)
{
  var countryArr;
  
  var region = targetRegion.options[targetRegion.selectedIndex].text;
  regionSelect = region;
  if (countries[region])
  {
...
function set_city_state(targetCountry)
{
  var city_stateArr;
  var country = targetCountry.options[targetCountry.selectedIndex].text;
  ...
  alert("Cannot found this country!");
  return ""; 
}

function print_city_state(targetState)
{
  
  var city_state = targetState.options[targetState.selectedIndex].text;
  stateSelect = city_state;
  if (stateSelect && city_states[countrySelect].indexOf(stateSelect) != -1){
    alert(regionSelect +' '+ stateSelect + ', ' + countrySelect);
    //document.getElementById('txtplacename').innerHTML = regionSelect +' '+ stateSelect + ', ' + countrySelect;
  }
  else {
     //document.getElementById('txtplacename').innerHTML = countrySelect;
     alert(countrySelect);
  }
}
